  ...
  <div class="main">
    <el-tabs type="border-card" @tab-click="switchPage">
      ...
      <el-tab-pane label="已删除">
        <el-descriptions :title="'岗位名称: '+item.title" v-for="item, index in delZhaoPins" :key="index"
                         style="border: grey solid 1px; background-color:#d3dce6;margin-bottom:20px"
                         :content-style="contentStyle" :label-style="{fontSize:20}">
          ...
          <el-descriptions-item label="操作">
            <el-button :type="item.status==0?'primary':'danger'" size="small" @click="re(item.id,item.status)">
              {{ item.status | but }}
            </el-button>
          </el-descriptions-item>
        </el-descriptions>
      ...
      <el-tab-pane label="发布招聘">
        <el-descriptions title="发布招聘信息" style="border: grey solid 1px; background-color:#d3dce6"
                         :content-style="contentStyle" :label-style="{fontSize:20}">
          <el-descriptions-item label="岗位名称">
            <el-input v-model="zhaoPin.title" placeholder="请输入内容"></el-input>
          </el-descriptions-item>
          <el-descriptions-item label="工作地点">
            <el-input v-model="zhaoPin.address" placeholder="请输入内容"></el-input>
          </el-descriptions-item>
          <el-descriptions-item label="学历要求">
          ...
          <el-descriptions-item label="人数">
            <el-input v-model="zhaoPin.num" placeholder="请输入内容"></el-input>
          </el-descriptions-item>
          <el-descriptions-item label="岗位描述">
            <el-input type="textarea" autosize v-model="zhaoPin.describe" placeholder="请输入内容"></el-input>
          </el-descriptions-item>
          <el-descriptions-item label="岗位要求">
            <el-input type="textarea" autosize v-model="zhaoPin.text" placeholder="请输入内容"></el-input>
          </el-descriptions-item>
        </el-descriptions>
...
import ZhaoPinTable from '@/views/components/ZhaoPinTable.vue';
...
export default {
  ...
  methods: {
    ...
    upload(num) {
      ...
      axios.get('/prod-api/zhaoPin?num=' + num).then(res => {
    ...
    re(id, status) {
      ...
      axios.get('/prod-api/admin/zhaoPin/' + type + '?id=' + id).then(res => {
        if (res.data) {
          this.$notify({
            title: '成功',
            message: '成功',
    ...
    getDelPage(num) {
      axios.get('/prod-api/admin/zhaoPin/deletedPage').then(res => {
        this.delNumber = res.data
      })
      axios.get('/prod-api/admin/zhaoPin/deleted?num=' + num).then(res => {
