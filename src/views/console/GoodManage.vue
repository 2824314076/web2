          ...
          <el-collapse-item :name="good.id" v-for="good in goods" :key="good.id">
            <template slot="title">
              ...
              <div style="float:left;width:10%">
                <el-tag>￥{{ good.price }}</el-tag>
              </div>
              <div style="float:left;width:10%">
                <el-button type="danger" size="small" @click="del(good.id)">删除</el-button>
              </div>
            </template>
        ...
        <div class="demo-input-suffix">
          <table style="width:100%;">
            <tr>
              ...
              <td>
                <el-input v-model="good.title" placeholder="请输入内容"></el-input>
              </td>
            </tr>
            <tr>
              <td>图片</td>
            ...
            <tr>
              <td>参考价</td>
              <td>
                <el-input v-model="good.price" placeholder="请输入内容"></el-input>
              </td>
            </tr>
            <tr>
              <td>设计师</td>
...
export default {
  ...
  mounted() {
    axios.get('/prod-api/admin/good/all').then(res => {
      this.goods = res.data
    })
    axios.get('/prod-api/designerIntroduce').then(res => {
  ...
  methods: {
    ...
    del(id) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        axios.get('/prod-api/admin/good/del?id=' + id).then(res => {
          if (res.data) {
            this.success()
            this.upload()
          } else this.failed()
