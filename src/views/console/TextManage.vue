...
export default {
  ...
  mounted() {
    ...
    axios.get('/prod-api/admin/all'+this.upName).then(res=>{
  ...
  methods: {
    submit() {
      this.$confirm('此操作将更新官网信息，是否继续?', '提示', {
        ...
        console.log(this.name)
        axios.post('/prod-api/admin/' + this.name + '/add', {[this.name]: this.text}).then(
            res => {
              if (res.data) {
                this.$notify({
                  title: '成功',
    ...
    updateModel() {
      axios.get('/prod-api/admin/' + this.name + '/all').then(res => {
        this.preText = res.data
      })
      this.isEdit = false
    },
