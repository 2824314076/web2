          ...
          <el-collapse-item :name="anLi.id" v-for="anLi in anLis" :key="anLi.id">
            <template slot="title">
              ...
              <div style="float:left;width:10%">
                <el-tag>{{ anLi.date }}</el-tag>
              </div>
              <div style="float:left;width:10%">
                <el-button type="danger" size="small" @click="del(anLi.id)">删除</el-button>
              </div>
            </template>
        ...
        <div class="demo-input-suffix">
          <table style="width:100%;">
            <tr>
              ...
              <td>
                <el-input v-model="anLi.title" placeholder="请输入内容"></el-input>
              </td>
            </tr>
            <tr>
              <td>图片</td>
...
export default {
  ...
  mounted() {
    axios.get('/prod-api/admin/anLi/all').then(res => {
      this.anLis = res.data
    })
  ...
  methods: {
    ...
    del(id) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        axios.get('/prod-api/admin/anLi/del?id=' + id).then(res => {
          if (res.data) {
            this.success()
            this.upload()
          } else this.failed()
