        ...
        <el-collapse accordion v-for='pic in pictures' :key="pic.id">
          <el-collapse-item name="1">
            <template slot="title">
              ...
              <div style="width:10%">
                <el-button type="danger" @click="del(pic.id)">删除</el-button>
              </div>
            </template>
...
export default {
  ...
  methods: {
    ...
    upload() {
      axios.get('/prod-api/banner').then(res => {
    ...
    push() {
      ...
      for (let i = 0; i < this.fileList.length; i++) {
        undefined
        axios.get('/prod-api/admin/banner/add?picture=' + this.fileList[i].url)
      }
      this.$notify({
        title: '成功',
        message: '发布成功',
    ...
    del(id) {
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        axios.get('/prod-api/admin/banner/del?id=' + id).then(res => {
          if (res.data) {
            this.$notify({
              title: '成功',
              message: '发布成功',
