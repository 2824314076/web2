<template>
  <div>
    <el-descriptions
        :title="'岗位名称: ' + item.title"
        v-for="(item, index) in zhaoPins"
        :key="index"
        style="
    ...
    >
      ...
      <el-descriptions-item label="操作">
        <el-button
            ...
            size="small"
            @click="re(item.id, item.status)"
        >{{ item.status | but }}
        </el-button
        >
        <el-button plain @click="showCeLan(item.id, item.title)"
        >应聘列表
        </el-button
        >
      </el-descriptions-item>
    ...
    <div style="margin-top: 20px; text-align: center">
      <el-button v-show="pageNumber > 2 && currentPage != 1" @click="upload(1)"
      >第一页
      </el-button
      >
      <el-button
...
<script>
import CeLan from '@/views/components/CeLan.vue'
...
export default {
  ...
  methods: {
    showCeLan(id, title) {
      axios.get('/prod-api/admin/yingPin/get?zhaoId=' + id).then((res) => {
